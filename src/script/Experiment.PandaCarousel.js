(function() {
	...
	function PandaCarouselInputPlugin(carousel, options) {
		...
		console.log('PandaCarouselInputPlugin created!');
		
	};
	
	PandaCarouselInputPlugin.prototype.initEvents = function() {
	...
	PandaCarouselInputPlugin.prototype.handleEvent = function(eventObject) {
	...
	PandaCarouselInputPlugin.prototype.startDraw = function(eventObject, id, x, y) {
	...
	PandaCarouselInputPlugin.prototype.extendDraw = function(eventObject, id, x, y, deltaX, deltaY) {
	...
	PandaCarouselInputPlugin.prototype.endDraw = function(eventObject, id) {
		...
		if (this.axisLocked && this.axis == this.AXIS_X) {
			...
			// Did we move enough?
			if (((Math.abs(this.distanceXY[id].x) / this.carousel.cacheElementWidth) * 100) > this.snapPercent) {
	...
	PandaCarouselInputPlugin.prototype.destroy = function() {
	...
	var standardCarousel = new PandaCarousel(document.getElementById('standard-carousel'), {
		childrenPerPage : 4,
		...
		plugins : {
			inputPlugin : PandaCarouselInputPlugin
		}
	});
