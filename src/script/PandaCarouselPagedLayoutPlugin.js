function PandaCarouselPagedLayoutPlugin(carousel, options) {
  ...
  this.transitionDurationProperty = this.getStyleProperty([
    'transitionDuration', 'WebkitTransitionDuration', 'MozTransitionDuration',
    'OTransitionDuration', 'msTransitionDuration', 'KhtmlTransitionDuration'
  ]);
  this.offset = 0;
  ...
  this.childrenPerPage =
    (options.childrenPerPage === undefined) ? 1 : options.childrenPerPage;
  this.horizontalLayout =
    (options.horizontalLayout === undefined) ? true : options.horizontalLayout;
  this.childCount = 0;
  this.childWidth = [];
  this.childHeight = [];
  this.childFromTop = [];
  ...
  this.preSetChildrenEventId = this.carousel.addEventListener(
    'presetchildren', this.setChildren.bind(this)
  );
}
...
/** Finish this */
PandaCarouselPagedLayoutPlugin.prototype.setChildren = function(children) {
  'use strict';

  var i, c, p, pl, cl;

  // Keep a note of the current page/child so we can try and keep them in
  // view if the layout changes
  var currentPage =
    (this.pages.length > 0) ? this.pages[this.currentPage] : null;
  var currentChild =
    (currentPage && currentPage.length) > 0 ? currentPage[0] : null;
  ...
  for (i = 0; i < this.childCount; i += this.childrenPerPage) {
  ...
  for (p = 0, pl = this.pageCount; p < pl; p++) {
    for (c = 0, cl = this.pages[p].length; c < cl; c++) {
      ...
      widthPx = this.horizontalLayout ?
        this.pageWidth / this.pages[p].length :
        this.pageWidth;
      heightPx = this.horizontalLayout ?
        this.pageHeight :
        this.pageHeight / this.pages[p].length;
      ...
      // We need to do this as a two step process because CSS percentage
      // transforms are relative to the size of the child, not the container.
      // This means that we need the 'real' (pixel) values from which we can
      // generate the relative percent values
  ...
  for (c = 0, cl = this.childCount; c < cl; c++) {
  ...
  var found = false;
  for (p = 0, pl = this.pageCount; p < pl && !found; p++) {
    for (c = 0, cl = this.pages[p].length; c < cl && !found; c++) {
...
PandaCarouselPagedLayoutPlugin.prototype.destroy = function() {
  ...
  this.carousel.removeEventListener(
    'presetchildren', this.preSetChildrenEventId
  );
