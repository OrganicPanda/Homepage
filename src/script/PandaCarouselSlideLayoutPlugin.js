...
function PandaCarouselSlideLayoutPlugin(carousel) {
  ...
  this.renderOffsetEventId = this.carousel.addEventListener(
    'renderoffset', this.render.bind(this)
  );
  this.renderEventId = this.carousel.addEventListener(
    'render', this.render.bind(this)
  );
  this.gotoPageNearestOffsetEventId = this.carousel.addEventListener(
    'gotopagenearestoffset', this.gotoPageNearestOffset.bind(this)
  );
...
PandaCarouselSlideLayoutPlugin.prototype.initPages = function() {
  ...
  this.pageElements = Array.prototype.slice.call(
    this.carousel.element.children,
    0
  );
...
PandaCarouselSlideLayoutPlugin.prototype.storeOriginalStyles = function() {
  ...
  for (e = 0; e < this.pageElements.length; e++) {
    this.pageElementsOriginalStyles.position[e] =
      this.pageElements[e].style.position;
...
PandaCarouselSlideLayoutPlugin.prototype.computeOffsetStyles = function() {
  ...
  // Translate those widths in to percentages based on the
  // current size of the carousel
  var totalPixelWidth = this.carousel.cacheElementWidth;
  var currentPercentWidth = 0;
  var currentPercentOffset = 0;
  for (e = 0; e < this.pageElements.length; e++) {
    currentPercentWidth =
      (100 / totalPixelWidth) * this.pageElementsPixelWidths[e];
...
PandaCarouselSlideLayoutPlugin.prototype.render = function(
  carouselOffsetX, carouselOffsetY) {
  'use strict';
  ...
  var carouselOffsetXPercent =
    (100 / this.carousel.cacheElementWidth) * carouselOffsetX;
  ...
  for (var e = 0; e < this.pageElements.length; e++) {
    this.pageElements[e].style.left = (
      this.pageElementsPercentOffsets[e] - targetOffset + carouselOffsetXPercent
    ) + '%';
  }
};

// Work out which page is closest to the given offset and go there
PandaCarouselSlideLayoutPlugin.prototype.gotoPageNearestOffset = function(
  carouselOffsetX, carouselOffsetY) {
  'use strict';
  ...
  var currentPageOffset = this.pageElementsPixelOffsets[
...
PandaCarouselSlideLayoutPlugin.prototype.destroy = function() {
  ...
  this.carousel.removeEventListener(
    'postupdatedimensions', this.postUpdateDimentionsEventId
  );
  this.carousel.removeEventListener(
    'renderoffset', this.renderOffsetEventId
  );
  this.carousel.removeEventListener(
    'render', this.renderEventId
  );
  this.carousel.removeEventListener(
    'gotopagenearestoffset', this.gotoPageNearestOffsetEventId
  );
